...
//! Command-line surface: a Go-`flag`-compatible parser for the flags upstream
//! envd accepts, including its exit codes and usage text. Unimplemented flags
//! are recognised, warned about and skipped; unknown flags are a usage error
//! (exit 2), because silently running on defaults is the failure mode this
