...
//! Command and input handlers for `process.Process`.
...
pub fn send_signal(
    ...
    let result = metadata::with_cause(&termination, "user", || {
        if signo == libc::SIGKILL {
            ...
            match cgroup.signal_all(signo) {
                ...
                Err(e) => {
                    ...
                    engine::kill_process_group(target, signo)
                }
            }
        } else {
            engine::kill_process_group(target, signo)
...
async fn write_process_input(
    ...
    match (&mut *writer, kind) {
        ...
        (engine::InputWriter::Pipe(None), InputKind::Stdin) => Err(ConnectError::new(
            ConnectCode::Internal,
...
mod tests {
    ...
    fn disabled_input() -> engine::InputHandle {
        Arc::new(tokio::sync::Mutex::new(engine::InputWriter::Pipe(None)))
    }
    ...
    fn get_proc_type_maps_pty_and_default() {
        let base = StartRequest {
            process: crate::process::wire::ProcessConfig {
                cmd: "x".into(),
                ..Default::default()
            },
            pty: None,
        ...
        let pty_req = StartRequest {
            pty: Some(crate::process::wire::Pty { size: None }),
            ..base
        };
    ...
    async fn pipe_input_and_close_stdin_reach_the_child() {
        ...
        loop {
                ...
                .expect("event bus closed before End")
            {
                ...
                engine::PumpEvent::End(end) => {
                    assert_eq!(end.exit_code, 0);
                    break;
                }
                engine::PumpEvent::SpawnError(e) => panic!("spawn error: {e}"),
                engine::PumpEvent::DeadlineExceeded => panic!("unexpected deadline"),
            }
        }
    ...
    async fn pty_input_uses_master_and_close_stdin_is_rejected() {
        ...
        while !String::from_utf8_lossy(&output).contains("got:hello") {
                ...
                .expect("event bus closed before expected output")
            {
                ...
                engine::PumpEvent::End(end) => panic!("PTY exited early: {end:?}"),
                engine::PumpEvent::SpawnError(e) => panic!("spawn error: {e}"),
                engine::PumpEvent::DeadlineExceeded => panic!("unexpected deadline"),
            }
        }
        ...
        loop {
            ...
            {
                engine::PumpEvent::End(_) => break,
                engine::PumpEvent::Data(_) => {}
