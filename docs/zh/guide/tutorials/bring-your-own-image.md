...
另有一个旋钮不是 flag，而是从 envd 自身的进程环境读取：
`CUBE_ENVD_CGROUP_MEMORY_MAX_BYTES`（daemon 管理的内存预算，单位字节）。上面的
`-cgroup-memory-max-bytes` flag 等价且优先，所以只能控制 `ENVD_EXTRA_ARGS` 的部署
可以用 `ENVD_EXTRA_ARGS="-cgroup-memory-max-bytes 268435456"`（256 MiB）设置它。
